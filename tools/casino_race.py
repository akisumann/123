...
**脚(四区画それぞれの重み)は非公開で、倍率に反映されない。** 出走表に載るのは系統と
地形だけなので、どの獣がどこで動くかを覚えている常連だけが、倍率の歪みを取りに行ける。

使い方:
    python3 tools/casino_race.py --day 93 --race 3            # 出走表(客が見る情報)
    python3 tools/casino_race.py --day 93 --race 3 --run      # 本番を走らせる
    python3 tools/casino_race.py --day 93 --race 3 --run --reveal   # 脚も見る(GM用)
    python3 tools/casino_race.py --list                       # 幻獣22種の隠し脚一覧
"""
...
# 脚(区画ごとの重み)は BEASTS 側に種ごとに持たせる。合計は全種4.00で揃えてあり、
# 強弱ではなく「どこで動くか」の差だけになる。非公開なので倍率には載らない。
...
# --- 幻獣22種 -----------------------------------------------------------------
# (種族, 系統[公開], 脚[非公開], 出走名に使う和語の候補)
#
# 系統は`world/crossroad/29`の「幻獣一覧」の地形適性から。
# 脚は同じ行の走りの説明を四区画へ数値化したもので、**合計は全種4.00で揃えてある**。
# 速い遅いの差ではなく「どこで動くか」の差だけを持たせるため。
#   第一=平地直線の初速／第二=地形／第三=地形＋妨害／第四=平地コーナーの伸び
BEASTS = [
    # 平地直線と終盤の追い込み。安定型
    ("狼",     "草原系", (1.05, 0.92, 0.98, 1.05), ["夜駆け", "群れ影", "追込"]),
    # 草原・森道・跳躍に強い。障害物を越えやすい
    ("鹿",     "森道系", (0.90, 1.15, 1.05, 0.90), ["跳躍", "夜鹿", "枝角"]),
    # 風・空中・高低差に強い。狭い地形は苦手
    ("鷹",     "岩場系", (1.15, 1.02, 0.88, 0.95), ["風羽", "高翔", "断崖"]),
    # 水路・ぬかるみ・曲線に強い。直線勝負は苦手
    ("蛇",     "水場系", (0.85, 1.12, 1.00, 1.03), ["沼這い", "曲身", "水縄"]),
    # 遅いが妨害に非常に強い。荒れるレースで残る
    ("亀",     "砂地系", (0.85, 0.92, 1.25, 0.98), ["鈍甲", "不動", "重殻"]),
    # 幻影・分岐・罠・曲がり道に強い。順位変動を起こす
    ("狐",     "霧幻系", (0.90, 0.98, 1.12, 1.00), ["幻惑", "七化け", "分かれ道"]),
    # 平地直線の速度が高い。第一区画で先行する
    ("馬",     "草原系", (1.22, 0.95, 0.90, 0.93), ["疾走", "直線", "先掛け"]),
    # 突破力が高く岩場・荒地に強い。小回りは苦手
    ("虎",     "岩場系", (1.00, 1.15, 1.02, 0.83), ["荒岩", "突破", "牙走り"]),
    # 初速と小回りが非常に高い。妨害に弱い
    ("兎",     "草原系", (1.20, 0.95, 0.83, 1.02), ["跳ね耳", "初足", "小回り"]),
    # 岩場・砂地・壁面に強い。平地では中程度
    ("蜥蜴",   "砂地系", (0.93, 1.15, 1.00, 0.92), ["岩肌", "壁這い", "砂走り"]),
    # 水場・湿地・跳躍に強い。乾いた砂地は苦手
    ("蛙",     "泥濘系", (0.93, 1.15, 1.02, 0.90), ["湿地", "跳ね泥", "水搔き"]),
    # 水辺・岩場・横移動・妨害耐性。直線速度は低い
    ("蟹",     "水場系", (0.85, 1.05, 1.15, 0.95), ["横這い", "硬爪", "波打ち"]),
    # 森道・糸場・障害物・複雑地形に強い。開けた平地は苦手
    ("蜘蛛",   "森道系", (0.85, 1.12, 1.10, 0.93), ["糸掛け", "枝渡り", "八脚"]),
    # 崖・坂道・岩場・高低差に強い。平地では地味
    ("山羊",   "岩場系", (0.88, 1.20, 1.02, 0.90), ["崖登り", "岩角", "坂駆け"]),
    # 直線突破と妨害突破。曲がり道で膨らむ
    ("猪",     "草原系", (1.10, 1.00, 1.12, 0.78), ["突進", "藪破り", "直進"]),
    # 幻影・風・視界妨害に強い。物理妨害には弱い
    ("蝶",     "霧幻系", (0.95, 1.02, 0.98, 1.05), ["風紋", "霞翅", "幻影"]),
    # 水路では圧倒的。水場がないとかなり弱い(地形依存が最も極端)
    ("魚",     "水場系", (0.88, 1.25, 1.00, 0.87), ["水路", "銀鱗", "流れ乗り"]),
    # 全体的に高性能だが、暴走や失速が起こる
    ("小竜",   "砂地系", (1.10, 1.08, 0.82, 1.00), ["火花", "暴走", "小翼"]),
    # 属性地形に強い。合えば大きく伸び、逆なら弱い
    ("小精霊", "霧幻系", (0.95, 1.18, 0.95, 0.92), ["属光", "宿り火", "揺らぎ"]),
    # ぬかるみ・狭路・衝撃吸収。速度は低いが妨害で崩れない
    ("スライム", "泥濘系", (0.83, 1.08, 1.18, 0.91), ["泥被り", "衝撃殺し", "狭路"]),
    # 障害回避と着地。荒れたレースほど生き残る
    ("猫",     "森道系", (0.92, 1.00, 1.15, 0.93), ["着地", "身躱し", "曲がり"]),
    # 軽量で小回りが利く。終いに動く
    ("貂",     "泥濘系", (1.00, 0.95, 0.97, 1.08), ["軽身", "小抜け", "纏い"]),
]
...
def build_card(day: int, race: int):
    ...
    entries = []
    for i, (sp, fam, legs, words) in enumerate(field, 1):
        name = r.choice(words) + kana[i - 1]
        entries.append({"枠": i, "出走名": name, "種": sp, "系統": fam, "脚": legs})
...
def main() -> int:
    ...
    ap.add_argument("--reveal", action="store_true", help="非公開の脚と、その元になった読み勝率を表示する(GM用)")
    ap.add_argument("--list", action="store_true", help="幻獣22種の系統と隠し脚を一覧")
    args = ap.parse_args()

    if args.list:
        print("# 幻獣22種(系統は公開／脚は**非公開**。合計はどれも4.00)")
        print(f"{'種':<6}{'系統':<7}{'第一':>6}{'第二':>6}{'第三':>6}{'第四':>6}")
        for sp, fam, legs, _ in BEASTS:
            print(f"{sp:<6}{fam:<7}" + "".join(f"{w:>6.2f}" for w in legs))
    ...
    if args.reveal:
        ...
        for e, o, p in zip(entries, odds, true_p):
            legs = "／".join(f"{w:.2f}" for w in e["脚"])
            print(f"  {e['出走名']}({e['種']})　脚 {legs}　"
                  f"倍率の元になった読み勝率={p * 100:.1f}%")
        print("  ※上の勝率は脚を伏せて出したもの。本番は脚込みで走るのでズレる。")
