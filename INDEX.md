...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口6,000)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 2,583 |
...
| `towns/rockwell/06_gareth_dorn.md` | ロックウェル鉱山方の長ガレス・ドルン(Lv47・ドワーフ・54歳)。三基の浮遊するコーンビット(大楯並の円錐ドリル)を操り、盾にも攻めにも使う。三基を合わせるとトリコーンビットになり採掘性能を最大限に発揮する。どの切羽を誰に掘らせるかを毎朝決め、賃金と順番の揉め事をその場で裁く。 | 3,235 |
...
合計 **174 ファイル / 1,728,120 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
