...
| `towns/rockwell/12_party.md` | 作成中。ロックウェル唯一の冒険者一党(チーム名未定)。B帯4人で町の最高戦力だが深層には届かない。半分固定のなんでも屋。メンバーは未設定。 | 1,256 |
...
合計 **180 ファイル / 1,747,613 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
