...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口6,000)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 2,635 |
...
| `towns/rockwell/07_rockwell_guild.md` | 冒険者ギルドのロックウェル支部。街規模なので出張受付所ではなく支部。職員は5〜6人、登録冒険者およそ200人。分布は壺型でD帯(約60)が一番厚く、F帯は細くB帯は増えつつある。A帯にあたる大モグラ坑道の深層(Lv50〜60)の依頼は出さずクロスロード支部へ送る。買い取りの本体は魔物素材とダンジョン産の肉。 | 2,989 |
...
合計 **175 ファイル / 1,737,613 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
