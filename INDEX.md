...
| `world/09_dungeon_generation.md` | 自然発生・遺構・人工など、ダンジョンを生成するルール。どのダンジョンにもコアがあり(作り出すクリエイトコア／忘れさせるオブリビオンコア／操るパペッターコア／大きくするマグニファイコア)、壊すと止まるので収支の面で壊さない。半起動という状態もある。 | 3,581 |
...
| `towns/rockwell/04_rockwell_mogura_tunnel.md` | ロックウェル固有のダンジョン・大モグラ坑道(全25階、浅層Lv20〜30/中層35〜45/深層50〜60)。入口は人の掘った四角い坑道で奥は獣の丸い穴。オオミミズ・オオモグラ・大アリの三者が掘り続けるので地図が保たない。中枢のマグニファイコアに近いほど個体が大きい。素材だけでなく栄養価の高い肉が採れ、町の食卓を実際に支えている。 | 3,162 |
...
合計 **172 ファイル / 1,710,734 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
