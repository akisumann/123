...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 鉱山町ロックウェル(街規模・人口6,000)の総合設定。クロスロードから2〜3日、街道は2本(金属を出す灰岩街道と、坑木・炭が入る南東街道)。鉱山は領主家の物で運営は町、首長は現地上がりの町長。名ありNPCは10人枠。 | 1,819 |
...
| `towns/rockwell/05_rockwell_facilities.md` | ロックウェルの組織と施設。鉱山はクロスロード領主家の物で、運営は町が任されている。長は現地上がりの町長(ガレス)。組織は代官所(町の役所)・鉱山方・荷方・神殿の四つ。 | 998 |
...
| `towns/rockwell/09_rockwell_works.md` | ロックウェルの採掘場と精錬施設。普通の金属が全種類出る(魔法金属は出ない)。稼働坑口12本・炉14基・鉱業2,000人。炉は第三階位の魔法で焚き、金属はクロスロードへ一括納入。木炭は町で焼き、原料の木は南東街道から入る。 | 1,516 |
...
合計 **180 ファイル / 1,748,087 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
