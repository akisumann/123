...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷の入口に張り付いた鉱山町ロックウェル(街規模・人口2,500)の総合設定。峡谷が崩落で枯れず危険度も下がらないので「危険なまま採り続けられる」のが商売の土台。掘る者と運ぶ者の町。クロスロードとファイルを分ける線引きもここ。 | 2,013 |
| `towns/rockwell/02_rockwell_town.md` | ロックウェルの町の作り。区画ではなく坑口から街道へ下る一本の坂で、上から坑口・精錬所・鍛冶場・宿と市・荷置き場。炉は止まらず町のどこからでも煙が見える。宿は峡谷の前泊後泊で二山、昼は空く。ギルドは出張窓口だけで依頼を起こす権限は無い。 | 1,405 |
| `towns/rockwell/03_rockwell_surroundings.md` | ロックウェルから見た周辺。灰岩峡谷(入口に町がある/道の情報が一番新しい)、採石場周りの浅い岩場(Lv10〜20で駆け出しの行き先)、忘れられた鉱山(隣だが町は食っておらず「あそこは鉱山ではない」)。大8の受け入れ先として宿が埋まり物価が上がる。 | 1,656 |
| `towns/rockwell/04_rockwell_and_crossroad.md` | ロックウェルとクロスロードの関係。掘って精錬する町と加工して売る街で上下ではない。荷の流れ(峡谷→一次精錬→灰岩街道→灰岩門→南区)。依頼はクロスロードで受けて現地入りする。二つの町を混ぜないための書き分けの規則も。 | 1,652 |
...
合計 **172 ファイル / 1,707,581 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
