...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 鉱山町ロックウェル(街規模・人口6,000)の総合設定。灰岩街道を南へ下りきった所にある行き止まりの町で、遠いのは危険ではなく距離。未設定の一覧もここ。 | 3,593 |
| `towns/rockwell/02_rockwell_town.md` | ロックウェルの町の作り。区画ではなく坑口から街道へ下る一本の坂。飯・酒・宿は相場のまま。 | 1,262 |
| `towns/rockwell/03_rockwell_danger_zones.md` | ロックウェル固有の危険地域。捨石丘陵(Lv10〜20・屑石の人工丘・駆け出しの出発点)と雷鳴山地(Lv25〜35・金属を持つと狙われる)。 | 2,046 |
| `towns/rockwell/04_rockwell_mogura_tunnel.md` | ロックウェル固有のダンジョン・大モグラ坑道(全25階・Lv20〜60)。四種が掘り続けるので地図が保たず、中枢のマグニファイコアで奥ほど個体が大きくなる。肉は町の食料庫。 | 3,608 |
| `towns/rockwell/05_rockwell_facilities.md` | ロックウェルの組織と施設。鉱山はクロスロード領主家の物で、町は代官が回している。組織は代官所・鉱山方・荷方・神殿の四つ。 | 2,227 |
| `towns/rockwell/06_gareth_dorn.md` | ロックウェル鉱山方の長ガレス・ドルン(Lv47・ドワーフ)。三基の浮遊コーンビットを操る、現場へ降りたがる管理職。差配は短く岩の話は長い。 | 2,868 |
| `towns/rockwell/07_rockwell_guild.md` | 冒険者ギルドのロックウェル支部。職員5〜6人・登録200人で分布は壺型、A級はいない。深層の依頼は出さずクロスロードへ送る。 | 2,473 |
| `towns/rockwell/08_rockwell_quest_board.md` | ロックウェル支部の定番依頼の掲示例。捨石丘陵(F〜E)・雷鳴山地(D〜C)・大モグラ坑道の浅層と中層(D〜B)。深層は掲示しない。 | 3,628 |
| `towns/rockwell/09_rockwell_works.md` | ロックウェルの採掘場と精錬施設。普通の金属が全種類出る(魔法金属は出ない)。稼働坑口12本・炉14基・鉱業2,000人。炉は第三階位の魔法で焚き、金属はクロスロードへ一括納入。 | 3,264 |
...
合計 **177 ファイル / 1,753,114 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
