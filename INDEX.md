...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口2,500)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 1,946 |
...
| `towns/rockwell/05_rockwell_mine.md` | ロックウェルが食っている稼働中の鉱山。冒険の場所ではなく、農家が畑へ出るのと同じ日常の労働。鉱夫はLv1〜20の一般人で、腕の差はレベルではなく年数で付く。掘り手・支え手・運び手・選り手・炉番に役が分かれ、屑石は捨石丘陵へ運ばれる。落石も落盤も起きるが事件ではない。 | 1,701 |
...
合計 **173 ファイル / 1,717,440 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
