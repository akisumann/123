...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 鉱山町ロックウェル(街規模・人口6,000)の総合設定。クロスロードから2〜3日、街道は2本(金属を出す灰岩街道と、坑木・炭が入る南東街道)。鉱山は領主家の物で運営は町、首長は現地上がりの町長。名ありNPCは10人枠。 | 1,805 |
...
| `towns/rockwell/12_party.md` | 作成中。ロックウェル唯一の冒険者一党(チーム名未定)。B帯4人で町の最高戦力だが深層には届かない。半分固定のなんでも屋。メンバーは未設定。 | 1,230 |
...
合計 **180 ファイル / 1,747,587 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
