...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口6,000)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 2,596 |
...
| `towns/rockwell/06_gareth_dorn.md` | ロックウェル鉱山方の長ガレス・ドルン(Lv44・ドワーフ・54歳)。どの切羽を誰に掘らせるかを毎朝決め、賃金と順番の揉め事をその場で裁く。HP:Sの体とDEX:Aの手が本体で、岩読みLv5は年数で覚えたもの。怪我そのものには怒らないが、無理をした経緯には怒る。町の気風はこの男の口から出ている。 | 2,882 |
...
合計 **174 ファイル / 1,725,790 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
