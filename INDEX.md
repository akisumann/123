...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `123_city.md` | 123_city — クロスロード編(単一ファイル版) | 680,754 |
...
| `rules/12_pc_creation.md` | 観覧モードからキャラを動かすモードへ移る時の手順。名前と性別を聞き、どんなステータスがいいかを大雑把に聞く。希望が無ければ三タイプを自動生成して選ばせる。 | 2,790 |
...
| `characters/pc_misaka.md` | 主人公・御坂美琴(Lv50)。北区拠点のBランク冒険者で雷使い。ほぼ固定砲台で、量で押すかわりに足が遅い。 | 959 |
...
合計 **167 ファイル / 1,559,769 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
