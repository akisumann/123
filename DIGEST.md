...
- **一つの案件で「話を大きくする」展開(後からの拡張)は0〜2回までに留める。** これは**拡張回数**の制限であって、案件の**規模**の制限ではない。小さく始まった依頼や出来事の裏に、次から次へとさらに大きな話を継ぎ足し、際限なく規模が膨らんでいく展開だけを禁止する。**初手から大事件・大抗争級のイベントとして発生させるのは全面的に有り**(拡張0回として扱う)。デカい案件をやりたい時は、小さな話を膨らませ続けるのではなく、最初からその規模の案件として出す。
