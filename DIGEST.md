...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口6,000)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 2,966 |
| `towns/rockwell/02_rockwell_town.md` | ロックウェルの町の作り。区画ではなく坑口から街道へ下る一本の坂で、上から坑口・選鉱場と精錬所・鍛冶場と住まい・宿と市・荷置き場。坂なので荷方の仕事は上りに集まる。炉は止まらず煙が細ければ何かあった合図。ギルドは職員5〜6人の小さな支部。 | 1,194 |
| `towns/rockwell/03_rockwell_danger_zones.md` | ロックウェル固有の危険地域。捨石丘陵(Lv10〜20、屑石を積み上げた人工の丘で今も成長中、戦闘より先に地面が敵になる、町の駆け出しの出発点)と雷鳴山地(Lv25〜35、鉱脈が帯電していて金属を持つと狙われる、鉱山町の隣なのに金属を持ち込めない山)。 | 2,078 |
| `towns/rockwell/04_rockwell_mogura_tunnel.md` | ロックウェル固有のダンジョン・大モグラ坑道(全25階、浅層Lv20〜30/中層35〜45/深層50〜60)。入口は人の掘った四角い坑道で奥は獣の丸い穴。四種が別々のやり方で掘り続けるので地図が保たない。中枢のマグニファイコアで個体が大きく肉も濃い。ダンジョン産というだけで栄養価が桁違いで、町は「ダンジョン産でないと物足りない」。奥のものは高級品。 | 4,290 |
...
| `towns/rockwell/07_rockwell_guild.md` | 冒険者ギルドのロックウェル支部。街規模なので出張受付所ではなく支部。職員は5〜6人、登録冒険者およそ200人。分布は壺型でD帯(約60)が一番厚く、F帯は細くB帯は増えつつある。A帯にあたる大モグラ坑道の深層(Lv50〜60)の依頼は出さずクロスロード支部へ送る。買い取りの本体は魔物素材とダンジョン産の肉。 | 3,089 |
| `towns/rockwell/08_rockwell_quest_board.md` | ロックウェル支部の定番依頼の掲示例。捨石丘陵(F〜E)・雷鳴山地(D〜C)・大モグラ坑道の浅層(D)と中層(C〜B)。深層(Lv50〜60)は掲示しない。雷鳴山地は金属を持ち込めないので装備の持ち替えが依頼の前提に入る。坑道の肉は常設買い取りなので依頼票になるのは大口だけ。依頼主が鉱山方や町の側であることがクロスロードとの違い。 | 4,087 |
| `towns/rockwell/09_rockwell_works.md` | 町の産業設備。鉄・銅・銀・鉛・錫と普通の金属が全種類出る(魔法金属は出ない。ミスリル系は火山帝国、灰岩鋼は灰岩峡谷の領分)。大規模採掘場は坑口が複数あり番号がそのまま何が出るかを指す(「今日は三番」)。大規模精錬施設は選鉱場と精錬所の二段で、金属ごとに炉が違うため複数の炉を交代で回して止めない。**炉は木炭ではなく第三階位の魔法で焚く超火力で、炉番はMPで選ばれる。**精錬した金属はまとめてクロスロードへ一括納入される。 | 3,604 |
...
合計 **177 ファイル / 1,751,229 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
