...
| `world/crossroad/80_stampede.md` | 危険地域の群れが一斉に街へ向かってくる都市防衛戦。城壁があるので籠城すれば都市は落ちないが、壁の内側から戦っても群れは減らず、被害は壁の外(畑・街道・農村・隊商)へ積み上がり続ける。籠城は領主の三信条を三つとも同時に止めるので、打って出るしかないという形が本体。数字は湧き場をそのまま使う(敵戦況値据え置き、回復役必須)。帯が足りなければ頭数で埋まり、2倍3倍で当たれば覆せるので、どの危険地域から来ても街の手で対処できる。赤牙＝数と兵站、灰岩＝飛ぶので壁の上が主戦場、骨鳴り＝一隊が倍以上になり浄化院が本業に回る。予兆は依頼票の形で五段階、危険地域が空になるのが決定打。 | 7,805 |
...
合計 **179 ファイル / 1,789,287 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
