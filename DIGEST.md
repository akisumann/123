...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `123_city.md` | 123_city — クロスロード編(単一ファイル版) | 763,348 |
...
| `world/29_world_seeds.md` | クロスロードの外、世界のどこかにあるものの種(ダンジョンコアを制御下に置いて人材を育てるダンジョン学園、大陸上を年中巡回する空中要塞、超大型多脚ゴーレムが運ぶ街、王国最強の特務部隊、黄泉に繋がるとされる奈落の渓谷)。75の世界規模版で、各項目は「決まっていること／既存設定との接続／仮置き・未決定」に分けてある。引き出しであってToDoではなく、結末は決めずGMの裁量に委ねる。 | 7,157 |
...
合計 **180 ファイル / 1,829,202 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
