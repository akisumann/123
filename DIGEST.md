...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口6,000)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 2,727 |
...
| `towns/rockwell/09_rockwell_works.md` | 町の産業設備。大規模なのは出口が一本しかないからで、掘った石をそのまま出せば街道一本では運びきれないため嵩と重さを落としてから出す。採掘場は坑口が複数あり番号で呼ぶ(「今日は三番」)。精錬は選鉱場と精錬所の二段で炉は止めない。街道一本・木炭・水の三つが町の産出の上限になっている。 | 2,269 |
...
合計 **177 ファイル / 1,746,722 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
