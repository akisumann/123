...
| `towns/rockwell/09_rockwell_works.md` | 町の産業設備。大規模採掘場は坑口が複数あり番号で呼ぶ(「今日は三番」)。大規模精錬施設は選鉱場と精錬所の二段で、複数の炉を交代で回して止めない。木炭と水を大量に使い、屑石も大量に出る。 | 1,562 |
...
合計 **177 ファイル / 1,747,087 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
