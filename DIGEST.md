...
| ファイル | 内容(TL;DR) | 文字数 |
|---|---|---|
| `towns/rockwell/01_rockwell.md` | 灰岩峡谷を越えた先にある鉱山町ロックウェル(街規模・人口6,000)の総合設定。自前の鉱脈で普通に栄えている。街道・峡谷・忘れられた鉱山はクロスロードと共有で、町固有の危険地域は町の先にある。クロスロードとファイルを分ける線引きもここ。 | 2,807 |
...
| `towns/rockwell/09_rockwell_works.md` | 町の産業設備。鉄・銅・銀・鉛・錫と普通の金属が全種類出る(魔法金属は出ない。ミスリル系は火山帝国、灰岩鋼は灰岩峡谷の領分)。大規模採掘場は坑口が複数あり番号がそのまま何が出るかを指す(「今日は三番」)。大規模精錬施設は選鉱場と精錬所の二段で、金属ごとに炉が違うため複数の炉を交代で回して止めない。**精錬した金属はまとめてクロスロードへ一括納入される。** | 2,893 |
...
合計 **177 ファイル / 1,749,872 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
