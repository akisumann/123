...
| `towns/rockwell/09_rockwell_works.md` | 町の産業設備。大規模なのは出口が一本しかないからで、掘った石をそのまま出せば街道一本では運びきれないため嵩と重さを落としてから出す。採掘場は坑口が複数あり番号で呼ぶ(「今日は三番」)。精錬は選鉱場と精錬所の二段で、複数の炉を回して止めない。木炭と水を大量に使う。 | 2,017 |
...
合計 **177 ファイル / 1,747,090 文字**(空白除く)。1ファイルずつ読めば、一度に扱う量は常に小さく保てる。
